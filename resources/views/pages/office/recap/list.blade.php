    ...
    <table class="table table-row-dashed table-row-gray-300 align-middle gs-0 gy-4">
        <thead>
            <tr class="fw-bolder text-muted">
                ...
                <th class="max-w-150px">Tugas</th>
                <th class="max-w-140px">Deskripsi</th>
                <th class="max-w-100px">harga</th>
        ...
        <tbody>
            ...
            <tr>
                ...
                <td>
                    <span class="fw-bolder text-dark">{{ $item->nama_tugas }}</span>
                </td>
                <td class="text pe-0">
                    <span class="fw-bolder text-dark">{{ $item->deskripsi_tugas }}</span>
                </td>
                <td class="text pe-0">
                    <span class="fw-bolder text-dark">{{ number_format($item->harga,0, ',', '.') }}</span>
                </td>
                <td class="text pe-0">
                    ...
                    <div class="btn-group" role="group">
                        ...
                        <div class="dropdown-menu menu menu-sub menu-sub-dropdown menu-column menu-rounded menu-gray-600 menu-state-bg-light-primary fw-bold fs-7 w-125px py-4" aria-labelledby="aksi">
                            <div class="menu-item px-3">
                                <a href="javascript:;" onclick="load_input('{{route('recap.edit',$item->id)}}');" class="menu-link px-3">Ubah</a>
                            </div>
                            <div class="menu-item px-3">
                                <a href="javascript:;" onclick="handle_delete('{{route('recap.destroy',$item->id)}}');" class="menu-link px-3">Hapus</a>
                            </div>
                            @if($item->status == 'Sedang Diproses')
                            <div class="menu-item px-3">
                                <a href="javascript:;" onclick="handle_confirm('Apakah Anda Yakin?','Yakin','Tidak','POST','{{route('recap.selesai',$item->id)}}');" class="menu-link px-3">Pesanan Selesai</a>
                            </div>
                            @endif
                        </div>
                    </div>
